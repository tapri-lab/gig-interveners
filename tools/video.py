...
def stitch_videos(
    ...
    individual_video_path: Path,
    output_path: Path,
    audio_paths: List[Path] = [],
    audio_offset: float = 0.0,
):
    """
    Stitch multiple video files into a 2x2 grid with a vertical side video,
    ...
    :param individual_video_path: Vertical side .mp4 video file
    ...
    """
    input_files = list(global_video_path.rglob("*.mp4"))
    ...
    side_scaled = side_stream.video.filter("scale", -1, "ih*2+20").filter(
