...
@app.cell
def _(np):
    data = np.load("./demo_data/world_positions.npz")
    data.keys()
    return (data,)
...
@app.cell
def _(data, plt, x_):
    plt.plot(x_, data['LeftHand'][:, 2])
    return


@app.cell
def _(data, plt, x_):
    plt.plot(x_, data['LeftHand'][:, 1])
    return
...
def _(data, plt):
    ...
    lh = data["LeftHand"]
    # Plot the 3D line
    # Assuming you have x, y, z coordinates in arrays
    ax.plot3D(lh[:, 0], lh[:, 1], lh[:, 2], 'blue')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    return ax, fig, lh
...
@app.cell
def _(einops, lh):
    lh2 = einops.rearrange(lh, "n d -> d n")
    return (lh2,)


@app.cell
def _(lh2, recurrence_matrix):
    rm = recurrence_matrix(lh2, 0.2)
    rm.shape
    return (rm,)


@app.cell
def _(rm, rqa_metrics):
    rec, det, mean_length, max_length = rqa_metrics(rm)
    return det, max_length, mean_length, rec
...
def _(LinearRegression, copy, np, scipy):
    """Synchrony Metrics
    ...
    return (
        ...
        get_driver_scores,
        get_empath_scores,
        get_matrix_sync_coef,
        get_sync_coef,
        get_sync_index,
        kuramoto_weak_null,
        metric_fixed_parameters,
        recurrence_matrix,
        rho,
        rqa_metrics,
        shuffle_recordings,
        shuffle_time_windows,
        sum_normalized_csd,
        symbolic_entropy,
    )
